...
void loop() {
  ...
  int ret = armlink.ReadMsgs();
 
 if (ret == 2) {
   // passthru : gf
   int byret = armlink.passthru[0];
   int id = armlink.passthru[3];
   for (i = 1; i < armlink.passthru_length; i++) {
     ax12writeB(armlink.passthru[i]);
     setRX(id);
   }
   if (byret) {
     if(ax12ReadPacket(byret) > 0){
       sendret(ax_rx_buffer, byret);
     }
   }
  }
  ...
  if (ret == 1) {
    ...
    if (g_fArmActive) {
      ...
      if (fChanged && (g_bIKStatus != IKS_ERROR)) {
        MoveArmTo(sBase, sShoulder, sElbow, sWrist, sWristRot, sGrip, sDeltaTime, false);
      }
      else if (bioloid.interpolating && !paused) {
...
//    buttonsPrev = armlink.buttons;
    ...
    // no msg
    if (bioloid.interpolating > 0 && !paused) {
      bioloid.interpolateStep();
    }
  }
} 
